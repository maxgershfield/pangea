...
describe("TradesService", () => {
	...
	let _repository: Repository<Trade>;
	...
	beforeEach(async () => {
		...
		_repository = module.get<Repository<Trade>>(getRepositoryToken(Trade));
	...
	describe("findOne", () => {
		...
		it("should throw NotFoundException when trade does not exist", async () => {
			...
			await expect(service.findOne(tradeId, userId)).rejects.toThrow(NotFoundException);
		});
	...
	describe("findByUser", () => {
		...
		it("should apply asset filter", async () => {
			...
			expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith("trade.assetId = :assetId", {
				assetId: "asset-uuid",
			});
		});
		...
		it("should apply date range filters", async () => {
			...
			expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith("trade.executedAt >= :startDate", {
				startDate,
			});
			expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith("trade.executedAt <= :endDate", {
				endDate,
			});
		});
	});
	...
	describe("findByAsset", () => {
		it("should return trades for a specific asset", async () => {
			...
			expect(mockQueryBuilder.where).toHaveBeenCalledWith("trade.assetId = :assetId", { assetId });
			expect(result.total).toBe(1);
			expect(result.items).toHaveLength(1);
		});
	});
