...
export class TradesService {
	 ...
	 */
	async findByUser(userId: string, filters?: TradeFilters): Promise<TradeListResponseDto> {
		...
		queryBuilder.orderBy("trade.executedAt", "DESC").skip(skip).take(limit);
	 ...
	 */
	async findByAsset(assetId: string, filters?: TradeFilters): Promise<TradeListResponseDto> {
		const queryBuilder = this.tradeRepository
			.createQueryBuilder("trade")
			.leftJoinAndSelect("trade.buyer", "buyer")
			.leftJoinAndSelect("trade.seller", "seller")
	...
	async getStatistics(userId: string, assetId?: string): Promise<TradeStatisticsDto> {
		...
		return {
			totalTrades: Number.parseInt(result.totalTrades, 10) || 0,
