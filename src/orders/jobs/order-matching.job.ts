...
export class OrderMatchingJob {
	...
	async matchPendingOrders() {
		try {
			...
			this.logger.log(`Processing ${pendingOrders.length} pending order(s)`);
			...
			this.logger.error(`Error in order matching job: ${error.message}`, error.stack);
		}
	}
	...
	async matchOpenOrders() {
		try {
			...
			const openOrders = await this.orderRepository.find({
				where: [{ orderStatus: "open" }, { orderStatus: "partially_filled" }],
				relations: ["user"],
			...
			this.logger.error(`Error in open order matching job: ${error.message}`, error.stack);
		}
	}
}
