...
describe("OrderMatchingService", () => {
	...
	let _tradesService: TradesService;
	...
	beforeEach(async () => {
		...
		_tradesService = module.get<TradesService>(TradesService);
		...
		_webSocketService = module.get<WebSocketService>(WebSocketService);
	});
	...
	describe("findMatchingOrders", () => {
		it("should find matching sell orders for a buy order", async () => {
			...
			expect(mockQueryBuilder.where).toHaveBeenCalledWith("order.assetId = :assetId", {
				assetId: "asset-1",
			});
			expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith("order.orderType = :type", {
				type: "sell",
			});
			expect(mockQueryBuilder.orderBy).toHaveBeenCalledWith("order.pricePerTokenUsd", "ASC");
		});
		...
		it("should find matching buy orders for a sell order", async () => {
			...
			expect(mockQueryBuilder.orderBy).toHaveBeenCalledWith("order.pricePerTokenUsd", "DESC");
		});
	});
	...
	describe("processOrder", () => {
		...
		it("should process order and execute matches", async () => {
			...
			mockBalanceService.getPaymentTokenBalance.mockResolvedValue(
				BigInt(500_000_000) // 500 USD in smallest units
			);
	...
	describe("validateBalances", () => {
		it("should throw error if seller has insufficient balance", async () => {
			...
			await expect(
				service.validateBalances(buyOrder as Order, sellOrder as Order, 5)
