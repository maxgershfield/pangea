...
export class OrdersService {
	...
	private generateOrderId(): string {
		...
		const sequence = Math.floor(Math.random() * 100_000)
			.toString()
			.padStart(6, "0");
	...
	async create(dto: CreateOrderDto, userId: string): Promise<Order> {
		this.logger.log(`Creating ${dto.orderType} order for user ${userId}, asset ${dto.assetId}`);
		...
		if (dto.orderType === "sell") {
			await this.balanceService.lockBalance(userId, dto.assetId, BigInt(dto.quantity));
		}
		...
		try {
			...
			this.logger.error(`Error processing order ${savedOrder.orderId}: ${error.message}`);
			// If matching fails, unlock balance if it was a sell order
			if (dto.orderType === "sell") {
				await this.balanceService.unlockBalance(userId, dto.assetId, BigInt(dto.quantity));
			}
	...
	async validateOrder(dto: CreateOrderDto, userId: string): Promise<void> {
		...
		if (dto.isMarketOrder && dto.pricePerTokenUsd) {
			this.logger.warn("Market order with price specified - price will be ignored");
		}
	}
	...
	async cancel(orderId: string, userId: string): Promise<Order> {
		...
		if (order.orderType === "sell") {
			await this.balanceService.unlockBalance(userId, order.assetId, order.remainingQuantity);
		}
	 ...
	 */
	async update(orderId: string, dto: UpdateOrderDto, userId: string): Promise<Order> {
		const order = await this.orderRepository.findOne({
			where: { orderId, userId },
		});
		...
		if (dto.quantity !== undefined && order.orderType === "sell") {
			...
			if (quantityDiff > 0) {
				// Need to lock more
				await this.balanceService.lockBalance(userId, order.assetId, quantityDiff);
			} else if (quantityDiff < 0) {
				// Can unlock some
				await this.balanceService.unlockBalance(userId, order.assetId, -quantityDiff);
			}
		...
		order.totalValueUsd = Number(order.remainingQuantity) * order.pricePerTokenUsd;
	...
	async findByUser(
		...
		const { status, orderType, assetId, page = 1, limit = 20 } = filters || {};
	...
	async getHistory(
		...
		const _historyStatuses = ["filled", "cancelled", "expired"];
