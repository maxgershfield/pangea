...
export class OrderMatchingService {
	...
	async processOrder(order: Order): Promise<void> {
		...
		try {
			...
			if (matches.length === 0) {
				...
				this.logger.log(`No matches found for order ${order.orderId}, setting status to open`);
			...
			this.logger.log(`Found ${matches.length} potential matches for order ${order.orderId}`);

			// 2. Execute matches
			for (const match of matches) {
				if (order.remainingQuantity === BigInt(0)) {
					this.logger.log(`Order ${order.orderId} fully filled, stopping matching`);
					break;
				}
			...
			this.logger.error(`Error processing order ${order.orderId}: ${error.message}`, error.stack);
	...
	async executeMatch(buyOrder: Order, sellOrder: Order): Promise<void> {
		...
		try {
			...
			this.logger.log(`Blockchain execution successful, transaction hash: ${transactionHash}`);
		} catch (error) {
			this.logger.error(`Blockchain execution failed: ${error.message}`, error.stack);
		...
		try {
			...
			this.logger.error(`Failed to create trade record: ${error.message}`, error.stack);
			// Note: In production, you might want to handle this differently
			// (e.g., retry logic, compensation, etc.)
			throw new InternalServerErrorException(`Failed to create trade record: ${error.message}`);
		}
		...
		try {
			...
			this.logger.error(`Failed to transfer balances: ${error.message}`, error.stack);
			// Note: In production, you might want to implement rollback logic here
			throw new InternalServerErrorException(`Failed to transfer balances: ${error.message}`);
		}
		...
		try {
			const buyerBalance = await this.balanceService.getBalance(buyOrder.user.id, buyOrder.assetId);
			...
			this.logger.warn(`Failed to emit balance updates: ${error.message}`);
		}

		this.logger.log(`Match execution completed successfully: Trade ${trade.tradeId}`);
	}

	/**
	 * Validate balances before executing a trade
	 */
	async validateBalances(buyOrder: Order, sellOrder: Order, quantity: number): Promise<void> {
		this.logger.log(
