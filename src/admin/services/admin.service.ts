...
export class AdminService {
	...
	constructor(
		...
		@InjectDataSource() _dataSource: DataSource
	) {}
	...
	async getAssets(filters: AdminAssetFiltersDto) {
		const { page = 1, limit = 20, status, assetClass, blockchain, search } = filters;
		const skip = (page - 1) * limit;
	...
	async createAsset(dto: CreateAssetDto, issuerId?: string): Promise<TokenizedAsset> {
		...
		if (dto.totalValueUsd && dto.totalSupply) {
			asset.pricePerTokenUsd = Number((dto.totalValueUsd / Number(dto.totalSupply)).toFixed(2));
		}
	 ...
	 * Update asset (admin)
	 */
	async updateAsset(assetId: string, dto: UpdateAssetDto): Promise<TokenizedAsset> {
	...
	async getAssetStatistics(assetId: string) {
		...
		const [totalTrades, totalVolumeResult, averagePriceResult] = await Promise.all([
		...
		return {
			asset,
			totalTrades,
			totalVolume: totalVolumeResult?.total ? Number(totalVolumeResult.total) : 0,
			averagePrice: averagePriceResult?.avg ? Number(averagePriceResult.avg) : 0,
		};
	}
	...
	async getOrders(filters: AdminOrderFiltersDto) {
		const { page = 1, limit = 20, orderStatus, orderType, userId, assetId } = filters;
		const skip = (page - 1) * limit;
	...
	async cancelOrder(orderId: string): Promise<Order> {
		...
		if (order.orderStatus === "filled" || order.orderStatus === "cancelled") {
			throw new BadRequestException(`Cannot cancel order with status: ${order.orderStatus}`);
		}
	...
	async getOrderStatistics() {
		const [totalOrders, openOrders, filledOrders, cancelledOrders, totalVolume] = await Promise.all(
			[
	...
	async getTradeStatistics() {
		const [totalTrades, totalVolume, totalFees, averageTradeSize] = await Promise.all([
		...
		return {
			...
			averageTradeSize: averageTradeSize?.avg ? Number(averageTradeSize.avg) : 0,
		};
	}
	...
	async getAnalytics(filters: AnalyticsFiltersDto) {
		const { period = AnalyticsPeriod.MONTH, startDate, endDate, assetId } = filters;
		...
		const [tradeCount, tradeVolume, tradeFees] = await Promise.all([
			...
			tradeQueryBuilder.clone().select("SUM(trade.totalValueUsd)", "total").getRawOne(),
			tradeQueryBuilder.clone().select("SUM(trade.platformFeeUsd)", "total").getRawOne(),
		]);
