...
export class SmartContractService {
	...
	private readonly baseUrl = process.env.SMART_CONTRACT_GENERATOR_URL || "http://localhost:5000";
	...
	async generateRwaToken(spec: RwaTokenSpec): Promise<DeployResult> {
		...
		try {
			...
			const result = await this.generateCompileDeploy(contractSpec, "Rust", "RWA Token");

			this.logger.log(`✅ RWA Token contract deployed: ${result.contractAddress}`);
			return result;
		} catch (error) {
			this.logger.error(`Failed to generate RWA token contract: ${error.message}`, error.stack);
	...
	async deployOrderBook(): Promise<DeployResult> {
		...
		try {
			...
			const result = await this.generateCompileDeploy(contractSpec, "Rust", "Order Book");

			this.logger.log(`✅ Order Book contract deployed: ${result.contractAddress}`);
			return result;
		} catch (error) {
			this.logger.error(`Failed to deploy Order Book contract: ${error.message}`, error.stack);
			throw new HttpException(
	...
	async deployTradeExecution(): Promise<DeployResult> {
		...
		try {
			const contractSpec = {
				...
				framework: "Anchor",
				spec: {
					features: ["execute_trade", "settle_trade", "get_trade_history", "calculate_fees"],
					fee_structure: {
			...
			const result = await this.generateCompileDeploy(contractSpec, "Rust", "Trade Execution");

			this.logger.log(`✅ Trade Execution contract deployed: ${result.contractAddress}`);
			return result;
		} catch (error) {
			this.logger.error(`Failed to deploy Trade Execution contract: ${error.message}`, error.stack);
			throw new HttpException(
	...
	async deployVault(): Promise<DeployResult> {
		...
		try {
			...
			const result = await this.generateCompileDeploy(contractSpec, "Rust", "Vault");

			this.logger.log(`✅ Vault contract deployed: ${result.contractAddress}`);
			return result;
		} catch (error) {
			this.logger.error(`Failed to deploy Vault contract: ${error.message}`, error.stack);
			throw new HttpException(
	...
	private async generateCompileDeploy(
		...
		for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
			try {
				...
				const compiledBlob = await this.compileContract(generatedBlob, language, contractName);
				this.logger.log(`[${contractName}] ✅ Contract compiled`);

				// Step 3: Deploy contract
				const deployResult = await this.deployContract(compiledBlob, language, contractName);
				this.logger.log(`[${contractName}] ✅ Contract deployed`);

				return deployResult;
			} catch (error) {
				lastError = error as Error;
				this.logger.warn(`[${contractName}] Attempt ${attempt} failed: ${error.message}`);

				if (attempt < this.maxRetries) {
					const delay = this.retryDelay * attempt;
					this.logger.log(`[${contractName}] Retrying in ${delay / 1000}s...`);
	...
	private async generateContract(
		...
		try {
			const response = await axios.post(`${this.baseUrl}/api/v1/contracts/generate`, formData, {
				headers: formData.getHeaders(),
				responseType: "arraybuffer",
				timeout: 300_000, // 5 minutes for generation
			});

			return Buffer.from(response.data);
		} catch (error) {
			const axiosError = error as AxiosError;
			throw new Error(`Contract generation failed: ${this.getErrorMessage(axiosError)}`);
		}
	}
	...
	private async compileContract(
		...
		try {
			...
			const response = await axios.post(`${this.baseUrl}/api/v1/contracts/compile`, formData, {
				headers: formData.getHeaders(),
				responseType: "arraybuffer",
				timeout: 1_200_000, // 20 minutes for compilation
			});
			...
			if (axiosError.code === "ECONNABORTED") {
				throw new Error("Compilation timeout. The API may be under heavy load. Please try again.");
			}
			throw new Error(`Contract compilation failed: ${this.getErrorMessage(axiosError)}`);
		}
	}
	...
	private async deployContract(
		...
		try {
			const response = await axios.post(`${this.baseUrl}/api/v1/contracts/deploy`, formData, {
				headers: formData.getHeaders(),
				timeout: 300_000, // 5 minutes for deployment
			});
			...
			const axiosError = error as AxiosError;
			throw new Error(`Contract deployment failed: ${this.getErrorMessage(axiosError)}`);
		}
	}
	...
	async getCacheStats(): Promise<any> {
		try {
			const response = await axios.get(`${this.baseUrl}/api/v1/contracts/cache-stats`);
