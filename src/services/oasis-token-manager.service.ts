...
export class OasisTokenManagerService implements OnModuleInit {
	...
	constructor(
		...
		this.adminUsername = this.configService.get<string>("OASIS_ADMIN_USERNAME") || "OASIS_ADMIN";
		this.adminPassword = this.configService.get<string>("OASIS_ADMIN_PASSWORD") || "Uppermall1!";
	...
	async getToken(): Promise<string> {
		try {
			...
			this.logger.error(`Failed to get OASIS token: ${error.message}`, error.stack);
			throw error;
		}
	}
	...
	async refreshToken(): Promise<TokenInfo> {
		try {
			this.logger.log(`Authenticating with OASIS API as ${this.adminUsername}...`);
			...
			this.logger.debug(
				"OASIS authenticate response structure:",
			...
			if (!token) {
				...
				this.logger.error(
					"Attempted paths: result.result.jwtToken, result.jwtToken, jwtToken, token"
				);
				throw new Error("No token received from OASIS API");
			}

			this.logger.debug(
				`Token extracted successfully. Length: ${token.length}, Prefix: ${token.substring(0, 20)}...`
			);
			...
			this.logger.log(`✅ OASIS API token refreshed successfully. Expires in ${expiresIn} minutes`);
			...
			throw new Error(`Failed to refresh OASIS API token: ${error.message}`);
		}
	}
	...
	private async cacheToken(tokenInfo: TokenInfo): Promise<void> {
		try {
			...
			if (ttl > 0) {
				await this.redis.setex(this.TOKEN_CACHE_KEY, ttl, JSON.stringify(tokenInfo));
