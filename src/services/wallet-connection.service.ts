...
export class WalletConnectionService {
	...
	async verifySolanaOwnership(
		...
		try {
			...
			this.logger.log(
				`Solana signature verification for ${walletAddress}: ${isValid ? "VALID" : "INVALID"}`
			);
	...
	async verifyEthereumOwnership(
		...
		try {
			...
			this.logger.log(
				`Ethereum signature verification for ${walletAddress}: ${isValid ? "VALID" : "INVALID"}`
			);
	...
	async verifyOwnership(request: WalletConnectionRequest): Promise<VerifyWalletResult> {
		...
		if (blockchain === "solana") {
			...
			throw new HttpException(`Unsupported blockchain: ${blockchain}`, HttpStatus.BAD_REQUEST);
		}
